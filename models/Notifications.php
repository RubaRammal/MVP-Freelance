...
FOOTER;
    ...
    public function SendSignupMail($data)
    {
        try{
            ...
            $subject  = APP_NAME ." Sign up Confirmation";
            $message .= "<p> Dear </p>" .$data['username'] ." ,";
            $message .= "<p><strong>Thanks For Signing Up please visit the bellow link for signing up </p>";
            $message .= "<a>".USER_URL ."login <a/>";
            if ($this->sendNotification($subject, $message, $data['recipient'])) {
                return true;
            }
        }catch(exception $e){
